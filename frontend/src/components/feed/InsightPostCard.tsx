...
export function InsightPostCard({ post, author }: { post: any, author: any }) {
  ...
  return (
    <div className="feed-card feed-card-insight p-[16px]">
      ...
      {/* Content text rendered with react-markdown */}
      <div className="mb-4">
        <div className={`text-[14px] text-foreground leading-[1.6] prose prose-sm max-w-none ${expanded ? '' : 'line-clamp-4'}`}>
          <ReactMarkdown>
            {post.content}
          </ReactMarkdown>
        </div>
      ...
      {post.tags && post.tags.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {post.tags.map((tag: string) => (
            <span key={tag} className="border border-purple-200 bg-purple-50/80 rounded-[12px] px-[8px] py-[2px] text-[12px] text-purple-700">
              {tag}
            </span>
          ))}
        </div>
